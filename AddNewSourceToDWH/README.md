# AddNewSourceToDWH
![avatar](./images/update_schema.png)

### 📚 Описание проекта:  
Цель проекта: интеграция новых источников данных и создание новой витрины посредством разработки скриптов, обеспечивающих инкрементальное обновление данных.

### 📁 Структура проекта

```
.
├── images/              # Изображения.
├── scripts/             # Скрипты.
├──.gitignore/           # Файлы, игнорируемые Git-ом.
└──  README.md/          # Информация о проекте.
```

### ♦️ Модель данных
![model](./images/model.png)
В схеме `dwh` — таблицы измерений для мастеров ручной работы `d_craftsman`, заказчиков `d_customer` и товаров `d_product`. Также там есть таблица фактов о продажах `f_order`, инкрементальная витрина по мастерам за отчётные периоды `craftsman_report_datamart` и `MATERIALIZED VIEW` с отчётом продаж по всему маркетплейсу `orders_report_materialized_view`.Схема `external_source — новая`. Там как раз находятся таблицы нового источника, которые нужно подключить к хранилищу.  

---

![source1](./images/source1.png)  
В схеме `source1` — широкая таблица первого источника `craft_market_wide`.  

---

![source2](./images/source2.png)  
В схеме `source2` — две ненормализованные таблицы: таблица мастеров и товаров `craft_market_masters_products` и таблица заказчиков и заказов `craft_market_orders_customers`.   

---

![source3](./images/source3.png)  
В схеме `source3` — три таблицы: мастеров `craft_market_craftsmans`, заказчиков `craft_market_customers`, заказов с товарами `craft_market_orders`.  

---

### 🔧 Cкрипты для добавление нового источника в DWH 
  - `SQL_dwh_external_source_upload.sql` - Перенос данных из источников из source1, source2, source3, external_source в dwh.  
  - `DDL_customer_report_datamart.sql` - DDL для витрины.   
  - `SQL_dwh_customer_report_datamart_update.sql` - Обновление витрины. 
